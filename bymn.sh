...
cd ~/fabric-samples/fabric-multi-orgs
function printHelp() {
  echo "Usage: "
...
function networkUp() {
  ...
  #  IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE_COUCH up -d 2>&1
  #else
  #  IMAGE_TAG=$IMAGETAG docker stack deploy --compose-file $COMPOSE_FILE $DOCKER_STACK 2>&1
  #fi
  #Newly added
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/fabric-multi-orgs --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/first-network/ledgers-backup
	...
	echo $PWD .........................
    rm -f docker-compose-org1peer0.yaml
    rm -f docker-compose-org1peer1.yaml
    rm -f docker-compose-org2peer0.yaml
    rm -f docker-compose-org2peer1.yaml
  fi
}
...
function replacePrivateKey() {
  ...
  cp docker-compose-org1peer0-template.yaml docker-compose-org1peer0.yaml
  cp docker-compose-org1peer1-template.yaml docker-compose-org1peer1.yaml
  cp docker-compose-org2peer0-template.yaml docker-compose-org2peer0.yaml
  cp docker-compose-org2peer1-template.yaml docker-compose-org2peer1.yaml
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-org1peer0.yaml
  
  cd crypto-config/peerOrganizations/org1.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-org1peer1.yaml
  
  cd crypto-config/peerOrganizations/org2.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-org2peer0.yaml
  
  cd crypto-config/peerOrganizations/org2.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-org2peer1.yaml
...
if [ "${MODE}" == "up" ]; then
  echo $PWD up the network.................
  networkUp
elif [ "${MODE}" == "down" ]; then ## Clear the network
  echo $PWD down the network.................
  networkDown
